...
import { createGovernanceProposal, signGovernanceProposal, GovernanceProposal } from '../defi/timelockGovernance';
...
export const InstitutionalValuationPortal: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'calculator' | 'charts' | 'roadmap' | 'whitepaper' | 'timelock_governance'>('calculator');
  const [chartMetric, setChartMetric] = useState<'price' | 'burn' | 'nodes' | 'market_cap'>('price');

  // Governance proposals state
  const [proposals, setProposals] = useState<GovernanceProposal[]>([
    createGovernanceProposal('Annual Burn Acceleration', 'annualBurnRate', '3.5%'),
    createGovernanceProposal('Cold Reserve Gold Rebalancing', 'porGoldWeight', '40%')
  ]);
  const [newPropTitle, setNewPropTitle] = useState<string>('Expand Sovereign Nodes');
  const [newPropParam, setNewPropParam] = useState<string>('maxValidatorQuorum');
  const [newPropVal, setNewPropVal] = useState<string>('500');
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-8 bg-slate-950 text-slate-100 rounded-2xl border border-slate-800 shadow-2xl">
      ...
      <div className="flex items-center gap-2 border-b border-slate-800 pb-2 overflow-x-auto">
        ...
        <button
          onClick={() => setActiveTab('timelock_governance')}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs sm:text-sm font-medium transition-all ${
            activeTab === 'timelock_governance'
              ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/30'
              : 'text-slate-400 hover:text-white hover:bg-slate-900'
          }`}
        >
          <Key className="w-4 h-4" /> 3-of-5 PQC Timelock Vault
        </button>
      </div>
      ...
      {/* TAB 5: 3-OF-5 PQC TIMELOCK GOVERNANCE */}
      {activeTab === 'timelock_governance' && (
        <div className="space-y-6 p-6 rounded-xl bg-slate-900/60 border border-slate-800 shadow-xl">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-slate-800">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-base font-bold text-white">3-of-5 Post-Quantum Timelock Governance Vault</h3>
                <p className="text-xs text-slate-400">48-hour mandatory delay queue with ML-DSA-87 guardian multi-signatures</p>
              </div>
            </div>
            <span className="px-3 py-1 bg-amber-950/60 border border-amber-800 text-amber-300 text-xs font-mono rounded-full">
              Quorum: 3 / 5 Signers Required
            </span>
          </div>
          ...
          <div className="p-4 bg-slate-950 rounded-xl border border-slate-800 space-y-4">
            <h4 className="text-xs font-bold text-slate-300 uppercase tracking-wider flex items-center gap-2">
              ...
              Propose Institutional Parameter Change
            </h4>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                <label className="text-[11px] text-slate-400 block mb-1">Proposal Title</label>
                <input
                  type="text"
                  value={newPropTitle}
              ...
              <div>
                <label className="text-[11px] text-slate-400 block mb-1">Target Parameter</label>
                <input
                  type="text"
                  value={newPropParam}
                  onChange={(e) => setNewPropParam(e.target.value)}
                  className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-xs text-slate-200 font-mono outline-none focus:border-amber-500"
                />
              </div>
              <div>
            ...
            <button
              onClick={() => {
                if (!newPropTitle || !newPropParam || !newPropVal) return;
                const prop = createGovernanceProposal(newPropTitle, newPropParam, newPropVal);
                setProposals([prop, ...proposals]);
              }}
              className="px-4 py-2 bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-500 hover:to-amber-600 text-white font-bold text-xs rounded-lg transition"
            >
              Submit to 48h Timelock Queue
            </button>
          </div>

          {/* Active Proposals List */}
          <div className="space-y-3">
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Active Governance Proposals ({proposals.length})</h4>
            {proposals.map((prop) => (
              <div key={prop.proposalId} className="p-4 bg-slate-950 rounded-xl border border-slate-800 space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div>
                    <span className="text-sm font-bold text-slate-200">{prop.title}</span>
                    <span className="text-[10px] text-slate-500 font-mono ml-2">({prop.proposalId})</span>
                  </div>
                  <span className={`px-2.5 py-0.5 rounded-full text-xs font-mono font-semibold self-start sm:self-auto ${
                    prop.status === 'QUEUED_TIMELOCK' ? 'bg-amber-950 text-amber-300 border border-amber-800' : 'bg-blue-950 text-blue-300 border border-blue-800'
                  }`}>
                    {prop.status} ({prop.signaturesCollected.length}/{prop.signaturesRequired} Sigs)
                  </span>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-mono bg-slate-900/70 p-2.5 rounded-lg text-slate-400">
                  <div>Parameter: <span className="text-slate-200">{prop.targetParameter}</span> &rarr; <span className="text-emerald-400">{prop.newValue}</span></div>
                  <div>Timelock Expiry: <span className="text-amber-400">{new Date(prop.timelockExpiryEpoch * 1000).toLocaleString()}</span></div>
                </div>

                <div className="flex items-center justify-between pt-1">
                  <span className="text-[11px] text-slate-500 font-mono">
                    Signatures: {prop.signaturesCollected.join(', ')}
                  </span>
                  {prop.signaturesCollected.length < prop.signaturesRequired && (
                    <button
                      onClick={() => {
                        const guardianId = `sig_guardian_pqc_${Math.floor(Math.random() * 900 + 100)}`;
                        const updated = signGovernanceProposal(prop, guardianId);
                        setProposals(proposals.map(p => p.proposalId === prop.proposalId ? updated : p));
                      }}
